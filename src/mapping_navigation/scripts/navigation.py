...
class Navigation:
        ...
        for point in self.path[closest_point+1:]:
            if point.segment_type != self.path[closest_point].segment_type and point.segment_type is not None:
                if self.car_state.calc_distance(point.coordinate[0], point.coordinate[1]) < 5:
                    rospy.loginfo(f'The following road segment is 5 meters or less ahead {RoadWarning(point.segment_type.value)}')
    ...
    def get_steering_angle(self) -> float:
        ...
        rospy.loginfo(f"Target point is {self.path[self.target_index].coordinate[0]}, {self.path[self.target_index].coordinate[1]}")
...
if __name__ == "__main__":
    map = load_from_file(os.path.abspath(os.path.dirname(__file__)) + "/coords.txt")
